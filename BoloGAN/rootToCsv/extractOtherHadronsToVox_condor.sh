...
### Run ./script.sh -b binning_file_vox.xml -p 130/321/2212/2112/ --etaSelection all
...
#inputFolderBase="/eos/atlas/atlascerngroupdisk/proj-simul/AF3_Run3/otherHadronsProduction/slicedFiles/pid"${pid}"_E"
inputFolderBase="/eos/atlas/atlascerngroupdisk/proj-simul/AF3_Run3/otherHadronsProduction/gridOutput/pid${pid}_E"
...
#TotalBatches=(1 1)
#energies=(1024 2048 4096 8192 16384)
...
do 
  ...
  #inputFolder=${inputFolderBase}${energy}"/" #only needed for sliced other hadrons 
  inputFolder=${inputFolderBase}${energy}"_eta_m500_500/NTUP/" #only needed for inclusive other hadrons
  ...
  do
    ...
    if [[ -f "existingFiles_run3_${binning_file_vox}/${fileNameToChekIfSampleAlreadyProcessed}.txt" ]]; then
    ...
    do
      ...
      if [[ ${useFiles} == "true" ]]; then
        inputData=${inputFolder}/m${eta_max}_m${eta_min}_${eta_min}_${eta_max}.root
